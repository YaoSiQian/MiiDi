...
def _make_comp():
    return Composition(
        ...
        tracks=[
            Track(
                name="Lead", program=73, role="melody", notes=[(0, 480, 60, 96), (480, 480, 64, 96)]
            )
        ],
    )
...
def test_judge_report_structure():
    report = JudgeReport(
        J1=80.0, J2=90.0, J3=70.0, per_item={"J1": [], "J2": [], "J3": []}, evidence=[]
    )
    d = report.to_dict()
    assert d["J1"] == 80.0
    assert d["J2"] == 90.0
...
def test_evaluate_judge_returns_report():
    ...
    client.respond_json.return_value = {
        "score": 75.0,
        "per_item": [
            {"item": "scale_adherence", "verdict": "yes", "evidence": "all notes in C major"}
        ],
