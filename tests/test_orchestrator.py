...
def lead_ok(_sys, _usr):
    return {
        "notes": [[o * 240, 240, p, 90] for o, p in enumerate([72, 74, 76, 72, 74, 76, 74, 72])]
    }  # spans 8 eighths=1920*... 8*240=1920 ticks = 1 bar
...
def test_run_pipeline_end_to_end_with_fake_llm(tmp_path):
    client = FakeClient(
        [BRIEF, lead_ok(None, None), bass_ok(None, None), NO_ADJUSTMENTS, {"track": None}]
    )
...
def test_revise_regenerates_track(tmp_path):
    ...
    client2 = FakeClient([{"layer": "track", "track": "Lead"}, quieter, {"track": None}])
...
def test_revise_regenerate_preserves_session_lineage(tmp_path):
    ...
    client2 = FakeClient(
        [{"layer": "regenerate"}, BRIEF, lead_ok(None, None), bass_ok(None, None), {"track": None}]
    )
...
def test_validation_gate_blocks_midi_for_oversized_piece(tmp_path):
    client = FakeClient(
        [BRIEF, lead_20_bars(None, None), bass_ok(None, None), NO_ADJUSTMENTS, {"track": None}]
    )
    result = run_pipeline("a long lead over a tiny form", "pop", client, out_dir=tmp_path)
    assert result.comp is not None
...
def test_self_review_breaks_immediately_when_invalid_at_round_start():
    ...
    comp = Composition(
        structure=[{"name": "verse", "start_bar": 0, "bars": 2}],
        tracks=[Track(name="L", role="melody", notes=[(0, 19200, 60, 96)])],
    )
    client = FakeClient([])
