...
from miidi.schema.model import Composition, KeySig, Meta, Track
...
def _make_comp(notes):
    return Composition(
        meta=Meta(
            ...
            bpm=120,
            time_signature=[4, 4],
            key=KeySig(tonic_pc=0, mode="major"),
            style="pop",
        ),
        structure=[{"name": "verse", "start_bar": 0, "bars": 4}],
        harmony=[],
        tracks=[Track(name="Lead", program=73, role="melody", is_drum=False, notes=notes)],
    )
...
def test_section_mute():
    ...
    result = apply_adjustments(
        comp, [{"action": "section_mute", "track": "Lead", "start_bar": 1, "end_bar": 3}]
    )
...
def test_octave_shift_up():
    ...
    result = apply_adjustments(
        comp, [{"action": "octave_shift", "track": "Lead", "direction": "up"}]
    )
    pitches = [n[2] for n in result.tracks[0].notes]
...
def test_octave_shift_down():
    ...
    result = apply_adjustments(
        comp, [{"action": "octave_shift", "track": "Lead", "direction": "down"}]
    )
    pitches = [n[2] for n in result.tracks[0].notes]
...
def test_unknown_track_ignored():
    ...
    result = apply_adjustments(
        comp, [{"action": "section_mute", "track": "Nonexistent", "start_bar": 0, "end_bar": 1}]
    )
