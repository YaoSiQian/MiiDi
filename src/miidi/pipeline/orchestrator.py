...
from miidi.pipeline.prompts import (
    classify_revision_system,
    classify_revision_user,
)
...
def _context_for(role: str, prior: dict, bar_ticks: int = 1920) -> str:
    if not prior:
        return ""
    if role in ("bass", "harmony"):
        mel = {n: t for n, t in prior.items() if t.role == "melody"}
        if mel:
            return build_context("full", mel, bar_ticks)
        return build_context("full", prior, bar_ticks)
    if role in ("counter", "color"):
        return build_context("pc_summary", prior, bar_ticks)
    return ""
...
def _compose_tracks(client, pack, brief, comp, specs, roles, log):
    ...
    for spec in ordered:
        ...
        comp = comp.model_copy(
            update={"tracks": [track if t.name == track.name else t for t in comp.tracks]}
        )
...
def run_pipeline(
    ...
    try:
        reviewed, trajectory = self_review(
            client, comp, pack.defaults, max_rounds=max_review_rounds
        )
    except Exception as exc:
        log.append(f"self-review failed: {exc}")
        reviewed, trajectory = comp, []
    else:
    ...
    violations = validate_composition(reviewed.clamp_to_boundary())
    if violations:
    ...
    if store is not None:
        store.save_version(sid, "reviewed", reviewed, {"trajectory": trajectory})
...
def revise(
    store: SessionStore, client, sid: str, feedback: str, out_dir: Path | None = None
) -> PipelineResult:
    ...
    if layer == "track" and target in track_names:
        ...
        context = f"{build_context('pc_summary', {t.name: t for t in latest.tracks}, latest.bar_ticks)}\nUSER FEEDBACK: {feedback}"
        track, _repairs = compose_track(client, pack, _brief_from_comp(latest), spec, context)
        updated = latest.model_copy(
            update={"tracks": [track if t.name == target else t for t in latest.tracks]}
        )
    ...
    if result.comp is not None and not gated:
        version = store.save_version(
            ...
            "revised-regenerated",
            result.comp,
            {"feedback": feedback, "trajectory": result.trajectory},
        )
        result.stage_log.append(f"saved revised-regenerated v{version} under {sid}")
    elif gated:
        result.stage_log.append(
            "regeneration rejected by whole-piece validation; nothing persisted"
        )
    result.sid = sid
    return result
...
def _specs_from(comp: Composition) -> list[InstrumentSpec]:
    role_hints = {"melody": "carry the tune", "bass": "support roots", "drums": "groove"}
    return [
        InstrumentSpec(
            name=t.name, program=t.program, role=t.role, description=role_hints.get(t.role, "")
        )
...
def _brief_from_comp(comp: Composition) -> MusicBrief:
    return MusicBrief(
        ...
        instruments=[
            InstrumentSpec(name=t.name, program=t.program, role=t.role, description="")
            for t in comp.tracks
        ],
    )
